...
export class WalletServiceUrlPage {
  ...
  ionViewWillEnter() {
    ...
    this.comment = this.replaceParametersProvider.replace(
      this.translate.instant(
        "{{appName}} depends on Stratis Wallet Service (SWS) for blockchain information, networking and synchronization. The default configuration points to https://stratisplatform.com."
      ),
      { appName }
    );
  ...
  public save(): void {
    ...
    switch (this.walletServiceForm.value.bwsurl) {
      case 'prod':
      case 'production':
        bws = 'http://bws.stratisplatform.com/bws/api';
        break;
      case 'sta':
      case 'staging':
        bws = 'http://bws.stratisplatform.com/bws/api';
      ...
      case 'loc':
      case 'local':
        bws = 'http://bws-test.stratisplatform.com/bws/api';
        break;
    }
    if (bws) {
