...
export class BitPayCardProvider {
  ...
  public sync(apiContext, cb) {
    ...
    this.bitPayProvider.post(
      ...
      data => {
        this.onGoingProcessProvider.clear();
        return cb(this._setError('Stratis Card Error: Get Debit Cards', data));
      }
    );
  }
      ...
      err => {
        ...
        this.getCards(data => {
          ...
          this.bitPayProvider.post(
            ...
            json,
            data => {
              this.logger.info('Stratis Get Invoices: SUCCESS');
              invoices = data.data || [];
              ...
              this.bitPayProvider.post(
                ...
                json,
                data => {
                  this.logger.info('Stratis Get History: SUCCESS');
                  history = data.data || {};
                ...
                data => {
                  this.logger.info(
                    'Error loading Stratis Card transaction history for ',
                    'card id: ',
                    cardId,
                    'Message: ',
                    data.error
                  ...
                  return cb(
                    this._setError('Stratis Card Error: Get History', data)
                  );
                }
              );
            },
            data => {
              this.logger.info(
                'Error loading Stratis Card invoice history for ',
                'card id: ',
                cardId,
                'Message: ',
                data.error
              ...
              return cb(
                this._setError('Stratis Card Error: Get Invoices', data)
              );
            }
          );
        });
      ...
      err => {
        ...
        this.getCards(data => {
          ...
          this.bitPayProvider.post(
            ...
            res => {
              if (res.error) {
                this.logger.error('Stratis TopUp: With Errors');
                return cb(res.error);
              } else {
                this.logger.info('Stratis TopUp: SUCCESS');
                return cb(null, res.data);
              }
            },
            res => {
              return cb(this._setError('Stratis Card Error: TopUp', res));
            }
          );
        });
      }
  ...
  public getInvoice(id, cb) {
    this.bitPayProvider.get(
      ...
      res => {
        return cb(this._setError('Stratis Card Error: Get Invoice', res));
      }
    );
  }
  ...
  public remove(cardId, cb) {
    this.persistenceProvider
      ...
      .catch(err => {
        this.logger.error('Error removing Stratis debit card: ' + err);
        return cb(err);
      });
  }
  ...
  public getRates(currency, cb) {
    this.bitPayProvider.get(
      '/rates/' + currency,
      data => {
        this.logger.info('Stratis Get Rates: SUCCESS');
        return cb(data.error, data.data);
      },
      data => {
        return cb(this._setError('Stratis Error: Get Rates', data));
      }
    );
  }
